(;    
;;  Paul Cooney 2021
;;  Applies a painterly effect to all open images with a fine brush
 )

(define (sfu-burshy-all-fine) 
...
(define (brushyFine 
    ...
    (gimp-context-set-brush-spacing 0.125)
    ...
    (set! y 0)
    (while (< y yMax)
        (scribbleFine img brushLayer backLayer x y width height)
        (gimp-displays-flush)
        (set! y (+ y 1))
        )
    (set! x (+ x 1))
...
(script-fu-register
  "sfu-burshy-all-fine"
  "<Image>/Filters/Brushify/Brushify-All-Fine"
