...
;;  Uses a thick brush with texture effects 
 )


(define (sfu-burshy-all-stamp) 
  (let* ((i (car (gimp-image-list))) 
    ...
    (while (> i 0) 
      ...
      (brushyThickStamp image drawable1)
...
(define (brushyThickStamp 
    ...
    (set! y spacing)
    (while (< y yMax)
        (scribbleThickStamp img brushLayer cclayer x y width height spacing)
        (gimp-displays-flush)
        (set! y (+ y spacing))
        )
    (set! x (+ x spacing))
    ...
    ; Create ThickBrushLayer
    (define thickBrushLayer(car (gimp-layer-copy brushLayer 0)))
    (gimp-item-set-name brushLayer "thickBrushLayer")
    (gimp-image-insert-layer img brushLayer 0 0)
    (gimp-displays-flush)
...
(define (scribbleThickStamp
    ...
    (define color)
    (define xstart(+ xi (rand spacing) halfNeg))
    (define ystart(+ yi (rand spacing) halfNeg))
    ...
    (define xend xend1)
    (define yend yend1)
    (define xcheck xcheck1)
    (define ycheck ycheck1)
    ...
    (gimp-clone drawTo colorFrom 0 xcheck ycheck 4 points)
...
(script-fu-register
  "sfu-burshy-all-stamp"
  "<Image>/Filters/Brushify/Brushify-All-Clone-Stamp"
  "brushstrokes all open images with the clone stamp tool"
  "Paul"
  "Paul"
  "May 2021"
  "RGB*"
